...
class Transcript{
	...
	public:	
		...
		std::string get_AAseq(){return AAseq;}
...
Transcript::Transcript(std::string& _str){
		...
		std::stringstream ss;
		ss << _str;    
 		ss >> chr >> transStart >> transEnd >> strand >> geneID >> geneName >> geneBiotype >> transID >> transName >> transBiotype >> proteinID >> RNAseq >> CDSseq >> AAseq1;
 		
 		// A slight cheat with the mapping between AA and DNA, read in each amino acid 3 times so the vector of 
 		// 	amino-acids is the same size as the genomic locus.
		AAseq = convertSingleToTripleAA(AAseq1);
}
...
int Nchecker_AA(std::string _seq){
        int flag = 0;
        std::string seq = _seq;
    ...
    while( getline(_data->input_transcriptfile,line) ){
        ...
        if(counterReg >= START[_data->me] && counterReg <= END[_data->me]){
            ...
            std::vector<int> Vpos; // genomic position of every nucleotide for the transcript
 			...
 			int cdsLength = trans.get_AAseq().length();
			...
			if(  Vpos.size() != cdsLength){std::cout << "Debug Vpos: " << trans.get_transID() << ", Vpos = " << Vpos.size() << " = " << cdsLength << std::endl;}
                ...
                for(std::multimap<std::string, Exon>::iterator it = range.first; it != range.second; ++it){
                    ...
                    if(it->second.get_rank() == rankCounter){
                    	...
                    	if(cdsFlag == 1){
	                        ...
	                        if( seqCounter < _data->depth || seqCounter + _data->depth + _data->k - 1 > cdsLength){ 
	                        	Binfo.exonboundSeq = "NA"; 
	                        } 
							else{ Binfo.exonboundSeq = remove_whitespace( trans.get_AAseq().substr( seqCounter - _data->depth, 2*_data->depth + _data->k - 1) ); }

							if(Binfo.exonboundSeq.length() != 2*_data->depth + _data->k - 1 && Binfo.exonboundSeq.length() != 2){ 
								Binfo.exonboundSeq = "NA"; 
							}
			...
			if( cdsLength < _data->k){ std::cout << trans.get_transID() << " CDS is less than " << _data->k << ", strand " << trans.get_strand() << std::endl;}
	                ...
	                for(int i(0); i < (cdsLength - _data->k)/3; i++){ 
	                    std::string key = convertTripleToSingleAA( trans.get_AAseq().substr(3*i, _data->k) );

	                    if(Nchecker_AA(key) == 0){
	                    	int value [4]  = {trans.get_chr(_data), trans.get_strand(), Vpos[3*i], Vpos[3*i + _data->k - 1]};
		                    kv->add( (char*) key.substr(0, _data->k/3).c_str(), sizeof(char)*_data->k/3, (char*) value, sizeof(int)*4);
...
void REDUCE_output(char *key, int keybytes, char *multivalue, int nvalues, int *valuebytes, MAPREDUCE_NS::KeyValue *kv, void *ptr){
	...
	if(nvalues == 1){
	    ...
	    _data->output_dist << std::string(key).substr(0,_data->k/3) << "\t" << Vvalues.size() << std::endl;	
	}
